...
void Sprite::UpdateMesh()
{
	...
	if (m_resourceChanged == true)
	{
		...
		{
			m_vertices = m_resource->GetSpriteVertices(m_position, m_yawDegrees + m_addedRotationForImage, m_scale, m_tint);
			m_indices = m_resource->GetTriangularIndices();
		}
	...
	{
		//Check to see if there was a change in one of the vertices.
		//If So, update the VBO.

		bool changed = false;
		std::vector<SpriteVert> verts = m_resource->GetSpriteVertices(m_position, m_yawDegrees + m_addedRotationForImage, m_scale, m_tint);
		if (verts.size() != m_vertices.size())
		{
			changed = true;
		}
		for (size_t i = 0; changed == false && i < m_vertices.size(); i++)
		{
			if (verts.at(i) != m_vertices.at(i))
			{
				changed = true;
			}
		}
