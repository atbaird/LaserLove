...
LayerInformation::LayerInformation(const size_t& id, const AABB2& boundingVolume, const Vector2& size)
	: m_layerID(id),
	m_enabled(true),
...
void SpriteRenderer::Update(const float& deltaSeconds)
{
	...
	const Vector2 camXY = m_cameraPosition.GetXY();
	const Vector2 screenMaxs = camXY + (m_virtualScreen / 2.f);
	const Vector2 screenMins = camXY - (m_virtualScreen / 2.f);
